#!/usr/bin/env python3
"""
RealSense YOLO-3D Fixed Version
Uses working color stream + depth estimation fallback
"""
...
class FixedRealSenseCamera:
    """
    Fixed RealSense camera that uses color stream + depth estimation fallback
    """
    ...
    def start(self):
        ...
        try:
            ...
            print(f"Fixed RealSense camera started successfully")
    ...
    def get_frames(self):
        """Get color frame and estimated depth - FIXED VERSION"""
        try:
            # Get color frame
            ...
            # The depth_estimator.estimate_depth returns (depth_map, color_frame)
            # but we only need the depth_map
            depth_result = self.depth_estimator.estimate_depth(color_frame)
...
def main():
    """Main function for fixed RealSense YOLO-3D."""
    ...
    # Output settings
    output_path = "fixed_realsense_output.mp4"
    ...
    print("=== Fixed RealSense YOLO-3D Object Detection ===")
    print(f"Using device: {device}")
    print("Using color stream + depth estimation fallback")
    
    # Initialize fixed camera
    print("Initializing fixed RealSense camera...")
    camera = FixedRealSenseCamera(
        width=camera_width,
    ...
    while True:
        ...
        try:
            ...
            # Add labels
            cv2.putText(combined_view, "3D Detection (Fixed)", (10, 30), 
            ...
            cv2.imshow('Fixed RealSense YOLO-3D', combined_view)
