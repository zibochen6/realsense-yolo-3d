#!/usr/bin/env python3
"""
RealSense YOLO-3D with proper 3D cubes - clean version without BEV
"""
...
class RealSense3DCubesClean:
    """
    RealSense camera with proper 3D cubes - clean version
    """
...
def draw_3d_cube_ultralytics_style(image, bbox_2d, depth_value, class_name, color=(0, 255, 0), thickness=2):
    """
    Draw a 3D cube in the style shown in the Ultralytics article
    This creates a proper 3D perspective cube with depth
    """
    try:
        x1, y1, x2, y2 = bbox_2d
        
        # Calculate cube dimensions based on depth
        # Closer objects have more pronounced 3D effect
        depth_factor = max(0.2, min(1.0, 3.0 - depth_value))  # Stronger effect for closer objects
        
        # Calculate 3D offsets
        width_2d = x2 - x1
        height_2d = y2 - y1
        
        # 3D depth offset - this creates the "depth" of the cube
        depth_offset = int(min(width_2d, height_2d) * 0.3 * depth_factor)
        depth_offset = max(20, min(depth_offset, 60))  # Reasonable range
        
        # Define the 8 corners of the 3D cube
        # Front face (original 2D bounding box)
        front_tl = (x1, y1)
        front_tr = (x2, y1)
        front_br = (x2, y2)
        front_bl = (x1, y2)
        ...
        # Front face (the original 2D bounding box)
...
def main():
    print("=== RealSense YOLO-3D with Proper 3D Cubes (Clean) ===")
    ...
    try:
        while True:
            ...
            if detections and depth_map is not None:
                for detection in detections:
                    try:
                        ...
                        # Draw proper 3D cube like in Ultralytics article
                        detection_frame = draw_3d_cube_ultralytics_style(detection_frame, bbox_2d, depth_value, class_name, color)
                        object_count += 1
            ...
            # Display
            cv2.imshow('RealSense YOLO-3D with Proper 3D Cubes', combined_view)
