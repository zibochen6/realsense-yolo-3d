#!/usr/bin/env python3
"""
RealSense YOLO-3D Working Version
...
class WorkingRealSenseCamera:
    """
    Working RealSense camera that uses color stream + depth estimation fallback
    """
    ...
    def start(self):
        ...
        try:
            ...
            print(f"Working RealSense camera started successfully")
...
def main():
    """Main function for working RealSense YOLO-3D."""
    ...
    # Output settings
    output_path = "working_realsense_output.mp4"
    ...
    print("=== Working RealSense YOLO-3D Object Detection ===")
    print(f"Using device: {device}")
    print("Using color stream + depth estimation fallback")
    
    # Initialize working camera
    print("Initializing working RealSense camera...")
    camera = WorkingRealSenseCamera(
        width=camera_width,
    ...
    while True:
        ...
        try:
            # Get frames from working camera
            ...
            # Add labels
            cv2.putText(combined_view, "3D Detection (Working)", (10, 30), 
            ...
            cv2.imshow('Working RealSense YOLO-3D', combined_view)
