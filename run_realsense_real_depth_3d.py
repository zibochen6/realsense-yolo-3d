...
Uses the working 6fps depth configuration and draws actual 3D cubes
"""
...
def draw_3d_cube(image, box_3d, color=(0, 255, 0), thickness=2):
    """
    Draw a proper 3D cube on the image using the 3D box parameters
    """
    try:
        ...
        offset_factor = min(1.0, max(0.1, 1.0 - depth_value))  # Closer objects have larger offset
        offset_x = int((x2 - x1) * 0.2 * offset_factor)
        offset_y = int((y2 - y1) * 0.2 * offset_factor)
        ...
        # Fill the top face with a semi-transparent color
        overlay = image.copy()
        ...
        # Fill the right face with a darker color
        ...
        depth_value = box_3d.get('depth_value', 0.0)
        ...
        cv2.putText(image, f"D:{depth_value:.2f}m", (x1, text_y), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        text_y -= 15
        
        cv2.putText(image, f"S:{score:.2f}", (x1, text_y), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
...
def create_bev_with_objects(boxes_3d, size=(400, 400)):
    ...
    for box_3d in boxes_3d:
        if box_3d['valid']:
            ...
            x = int(camera_x + center[0] * 40)  # Scale factor
            y = int(camera_y - depth * 40)  # Depth is forward
            ...
            if 0 <= x < size[0] and 0 <= y < size[1]:
                ...
                cv2.circle(bev_image, (x, y), 6, color, -1)
                cv2.putText(bev_image, class_name[:3], (x-10, y-10),
...
def main():
    print("=== RealSense YOLO-3D with REAL Depth and 3D Cubes ===")
                ...
                for detection in detections:
                    try:
                        ...
                        if box_3d['valid']:
                            ...
                            detection_frame = draw_3d_cube(detection_frame, box_3d)
