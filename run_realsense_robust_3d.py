#!/usr/bin/env python3
"""
RealSense YOLO-3D with Robust Real Depth and 3D Bounding Boxes
Tries real depth first, falls back to estimation if needed
"""
...
class RealSenseRobust3D:
    """
    RealSense camera with robust depth support and 3D bounding boxes
    """
    ...
    def start(self):
        ...
        try:
            ...
            try:
                ...
                # Test if depth stream works with a longer timeout
                ...
                print(f"⚠️ Depth stream not available: {e}, using fallback")
                self.real_depth_available = False
...
def main():
    print("=== RealSense YOLO-3D with Robust Real Depth and 3D Boxes ===")
    ...
    try:
        while True:
            ...
            # Display
            cv2.imshow('RealSense YOLO-3D with Robust Real Depth', combined_view)
