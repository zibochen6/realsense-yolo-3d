...
Uses the existing bbox3d_utils.py methods for proper 3D box drawing
"""
...
class RealSenseProper3D:
    ...
    def get_depth_at_point(self, x, y, depth_map):
        ...
        if depth_map is None or x < 0 or y < 0 or x >= depth_map.shape[1] or y >= depth_map.shape[0]:
            return 0.0
        
        depth_value = depth_map[y, x]
        if depth_value <= 0 or depth_value > 10.0:  # Filter invalid depths
            return 0.0
...
def main():
    print("=== RealSense YOLO-3D with Real Depth and Proper 3D Boxes ===")
    ...
    try:
        while True:
            ...
            # Update Bird's Eye View using the existing method
            if boxes_3d:
                try:
